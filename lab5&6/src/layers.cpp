...
{
    // (set 5): set output tensor descriptor out_shape to have format
    //               NCHW, be floats, and have dimensions n, c, h, w
    cudnnDataType_t dtype = CUDNN_DATA_FLOAT;
    CUDNN_CALL( cudnnSetTensor4dDescriptor(this->out_shape, CUDNN_TENSOR_NCHW,
                                               dtype, n, c, h, w) );
...
void Dense::forward_pass()
{
    ...
    // (set 5): out_batch = weights^T * in_batch (without biases)
    CUBLAS_CALL( cublasSgemm(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N,
                             out_size, batch_size, in_size,
                             &one,
                             this->weights, in_size,
                             this->in_batch, in_size,
                             &zero,
                             this->out_batch, out_size) );
...
void Dense::backward_pass(float learning_rate)
{
    ...
    // (set 5): grad_weights = in_batch * (grad_out_batch)^T
    CUBLAS_CALL( cublasSgemm(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T,
                             in_size, out_size, batch_size,
                             &one,
                             this->in_batch, in_size,
                             this->grad_out_batch, out_size,
                             &zero,
                             this->grad_weights, in_size) );
    ...
    // (set 5): grad_in_batch = W * grad_out_batch
    ...
    // (set 5): weights = weights + eta * grad_weights
    ...
    // (set 5): biases = biases + eta * grad_biases
    CUBLAS_CALL( cublasSaxpy(cublasHandle,
                             out_size,
                             &eta,
                             this->grad_biases, 1,
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    // (set 5): get descriptor of input minibatch, in_shape
    CUDNN_CALL( cudnnGetTensor4dDescriptor(prev->get_out_shape(), &dtype,
                                               &n, &c, &h, &w, &nStride, &cStride, &hStride, &wStride) );
    CUDNN_CALL( cudnnSetTensor4dDescriptor(in_shape, CUDNN_TENSOR_NCHW,
                                               dtype, n, c, h, w) );

    // (set 5): set descriptor of output minibatch, out_shape, to have the
    ...
    // (set 5): create activation descriptor, and set it to have the given
    //               activationMode, propagate NaN's, and have coefficient coef
    CUDNN_CALL( cudnnCreateActivationDescriptor(&this->activation_desc) );
    CUDNN_CALL( cudnnSetActivationDescriptor(this->activation_desc, CUDNN_ACTIVATION_RELU, CUDNN_PROPAGATE_NAN, 0.0f) );

}

Activation::~Activation()
{
    // (set 5): destroy the activation descriptor
    CUDNN_CALL( cudnnDestroyActivationDescriptor(this->activation_desc) );
}
...
void Activation::forward_pass()
{
    ...
    // (set 5): apply activation, i.e. out_batch = activation(in_batch)
    CUDNN_CALL( cudnnActivationForward(cudnnHandle,activation_desc,
            &one, this->in_shape, this->in_batch,
            &zero, this->in_shape, this->out_batch) );
}
...
void Activation::backward_pass(float learning_rate)
{
    ...
    //(set 5): do activation backwards, i.e. compute grad_in_batch
    CUDNN_CALL( cudnnActivationBackward(cudnnHandle,activation_desc,
                                       &one,
                                       this->out_shape, this->out_batch,
                                       this->get_out_shape(), this->grad_out_batch,
                                       this->in_shape, this->in_batch,
                                       &zero,
                                       this->get_in_shape(), this->grad_in_batch
                                       ) );
}
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    cudnnDataType_t dtype;
    int n, c, h, w, n_stride, c_stride, h_stride, w_stride;
    // (set 6): Get the input tensor descriptor in_shape into the variables
    //               declared above
    CUDNN_CALL( cudnnGetTensor4dDescriptor(prev->get_out_shape(), &dtype,
                                           &n, &c, &h, &w, &n_stride, &c_stride, &h_stride, &w_stride) );
    CUDNN_CALL( cudnnSetTensor4dDescriptor(in_shape, CUDNN_TENSOR_NCHW,
    ...
    //(set 6): Create & set a filter descriptor for a float array ordered
    //               NCHW, w/ shape n_kernels x c x kernel_size x kernel_size.
    //               This is class field filter_desc.

    CUDNN_CALL( cudnnCreateFilterDescriptor(&filter_desc) );
    CUDNN_CALL( cudnnSetFilter4dDescriptor(filter_desc, dtype,
                                           CUDNN_TENSOR_NHWC,
    ...
    // (set 6): Create and set a convolution descriptor. This will
    ...
    CUDNN_CALL( cudnnSetConvolution2dDescriptor(conv_desc, 0 ,0 , stride, stride, 1, 1,
    ...
    //TODO: CUDNN failure: CUDNN_STATUS_NOT_SUPPORTED
...
//    fwd_algo = CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD_NONFUSED;
//    bwd_filter_algo = CUDNN_CONVOLUTION_BWD_FILTER_ALGO_WINOGRAD_NONFUSED;
//    bwd_data_algo = CUDNN_CONVOLUTION_BWD_DATA_ALGO_WINOGRAD_NONFUSED;
...
Conv2D::~Conv2D()
{
    ...
    // (set 6): Destroy filter_desc and conv_desc
...
void Conv2D::forward_pass()
{
    ...
    // (set 6): Perform convolution forward pass (store in out_batch).
    ...
    CUDNN_CALL(cudnnConvolutionForward(cudnnHandle,
            &one, in_shape, in_batch, filter_desc, weights,
...
void Conv2D::backward_pass(float learning_rate)
{
    ...
    // (set 6): Compute the gradient with respect to the filters/weights
    ...
    CUDNN_CALL(cudnnConvolutionBackwardFilter(cudnnHandle,
            &one, in_shape, in_batch, out_shape, grad_out_batch, conv_desc, bwd_filter_algo, workspace, workspace_size,
            &zero, filter_desc, this->weights));
    ...
    // (set 6): weights = weights + eta * grad_weights
    CUBLAS_CALL( cublasSaxpy(cublasHandle,
                             this->n_weights,
                             &eta,
                             this->grad_weights, 1,
    ...
    // (set 6): biases = biases + eta * grad_biases
    CUBLAS_CALL( cublasSaxpy(cublasHandle,
                             n_biases,
                             &eta,
                             this->grad_biases, 1,
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    // (set 6): Create and set pooling descriptor to have the given mode,
...
Pool2D::~Pool2D()
{
    //(set 6): destroy the pooling descriptor
...
{
    float zero = 0, one = 1;
    // (set 6): do pooling in forward direction, store in out_batch
...
void Pool2D::backward_pass(float learning_rate)
{
    ...
    // (set 6): do pooling backwards, store gradient in grad_in_batch
...
: Loss(prev, cublasHandle, cudnnHandle)
{
    ...
    // (set 5): get descriptor of input minibatch, in_shape, into variables
    //               declared above
    CUDNN_CALL( cudnnGetTensor4dDescriptor(prev->get_out_shape(), &dtype,
                                           &n, &c, &h, &w, &nStride, &cStride, &hStride, &wStride) );
    CUDNN_CALL( cudnnSetTensor4dDescriptor(in_shape, CUDNN_TENSOR_NCHW,
...
void SoftmaxCrossEntropy::forward_pass()
{
    ...
    // (set 5): do softmax forward pass using accurate softmax and
    //               per instance mode. store result in out_batch.
    CUDNN_CALL( cudnnSoftmaxForward(cudnnHandle,CUDNN_SOFTMAX_ACCURATE,CUDNN_SOFTMAX_MODE_CHANNEL,
                                        &one, this->in_shape, this->in_batch,
                                        &zero, this->out_shape, this->out_batch) );
...
void SoftmaxCrossEntropy::backward_pass(float lr)
{
    ...
    // (set 5): first, copy grad_in_batch = out_batch
    CUBLAS_CALL( cublasScopy(cublasHandle, size, out_batch, 1, grad_in_batch, 1) );

    // (set 5): set grad_in_batch = grad_in_batch - grad_out_batch using
    //               cublasSaxpy
