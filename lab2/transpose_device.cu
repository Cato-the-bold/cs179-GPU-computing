...
void shmemTransposeKernel(const float *input, float *output, int n) {
    ...
    __shared__ float data[TILE_DIM][TILE_DIM];

    int i = threadIdx.x + 64 * blockIdx.x;
    int j = 4 * threadIdx.y + 64 * blockIdx.y;
    const int end_j =  4;

    for (int k=0; k<end_j; k++){
        data[4*threadIdx.y+k][threadIdx.x] = input[i + n * (j+k)];
    }
    ...
    for (int k=0; k<end_j; k++){
        output[i + n * (j+k)] = data[threadIdx.x][4*threadIdx.y+k];
    }
}
