...
__global__ void copy_vec(double *a, double *b) {
    //TODO write a cublas version in comments
    cublasStatus_t stat ; // CUBLAS functions status
    cublasHandle_t handle ;
    stat = cublasCreate (& handle );

    stat=cublasDcopy(handle,3, a,1, b,1);
...
//    d[2] = (a[2] * b) + c[2];
    float one = 1.0f;
    stat=cublasSaxpy(handle,3,&b, a,1, c,1);
...
//    c[1] = a[1] * b;
//    c[2] = a[2] * b;
    stat=cublasSscal(handle,3,&b, a,1);
...
//    c[2] = a[2] + b[2];
    ...
    stat=cublasSaxpy(handle,3, 1.0, a,1, b,1);
    cublasDcopy(handle,3, a,1, c,1);
...
//    c[2] = a[2] - b[2];
    ...
    stat=cublasSaxpy(handle,3, -1.0, b,1, a,1);
    cublasDcopy(handle,3, b,1, c,1);
}
...
//    c[2] = (b[6] * a0) + (b[7] * a1) + (b[8] * a2);
    ...
    cublasDgemv(handle,CUBLAS_OP_N,3,3,*alpha,b,3,a,1, 0.f, c,1);
}
...
//    return sqrt((vec[0] * vec[0]) + (vec[1] * vec[1]) + (vec[2] * vec[2]));
    double result;
    cublasSnrm2(handle,3 ,vec,1,&result);
...
__device__ void vec_scale(double *vec) {
    ...
    cublasSscal(handle,3, 1/n, vec,1);
    // vec * 1/n hint hint
//    vec[0] = vec[0] / n;
//    vec[1] = vec[1] / n;
//    vec[2] = vec[2] / n;
...
//    return (a[0] * b[0]) + (a[1] * b[1]) + (a[2] * b[2]);
    double result;
    cublasSdot(handle,3,a ,1, b ,1,&result);
}
